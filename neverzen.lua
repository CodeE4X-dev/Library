...
function NeverZen:CreateGradient(parent, colors, rotation, transparency)
...
function NeverZen:SetBlur(frame : Frame, NoAutoBackground, intensity)
...
function NeverZen.new(config)
	...
	-- Apply theme
	NeverZen:SetTheme(config.Theme);
	...
	if config.Acrylic then
		NeverZen:SetBlur(WindowFrame, false, 1.2)
	end;
    ...
    Textbox.BorderColor3 = Color3.fromRGB(0, 0, 0)
    Textbox.BorderSizePixel = 0
    Textbox.Size = UDim2.new(1, -5, 0, 45)
    ...
    InputFrame.Position = UDim2.new(0, 5, 0, 22)
    InputFrame.Size = UDim2.new(1, -10, 0, 20)

    -- Add gradient to input frame
    local inputGradient = NeverZen:CreateGradient(InputFrame, NeverZen.Theme.BackgroundGradient, 90)
    ...
    FocusIndicator.Parent = InputFrame
    FocusIndicator.AnchorPoint = Vector2.new(0, 1)
    FocusIndicator.BackgroundColor3 = NeverZen.Theme.Hightlight
    FocusIndicator.BorderSizePixel = 0
    FocusIndicator.Position = UDim2.new(0, 0, 1, 0)
    FocusIndicator.Size = UDim2.new(0, 0, 0, 2)

    UICorner_2.CornerRadius = UDim.new(1, 0)
    UICorner_2.Parent = FocusIndicator
    ...
    TextInput.Position = UDim2.new(0, 8, 0.5, 0)
    TextInput.Size = UDim2.new(1, -16, 0.8, 0)
    TextInput.ClearTextOnFocus = false
    TextInput.Font = Enum.Font.GothamMedium
    TextInput.PlaceholderText = config.PlaceholderText
    TextInput.PlaceholderColor3 = Color3.fromRGB(255, 255, 255)
    TextInput.Text = config.Default
    TextInput.TextColor3 = Color3.fromRGB(255, 255, 255)
    TextInput.TextSize = 13.000
    TextInput.TextTransparency = 0.200
    ...
    TextInput.Focused:Connect(function()
        ...
        TweenService:Create(FocusIndicator, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {
            Size = UDim2.new(1, 0, 0, 2)
        }):Play()
        
        TweenService:Create(Label, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {
            TextColor3 = NeverZen.Theme.Hightlight,
            TextTransparency = 0.05
        }):Play()
        
        TweenService:Create(TextInput, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {
            TextTransparency = 0.05
        }):Play()
    end)
    ...
    TextInput.FocusLost:Connect(function()
        ...
        config.Callback(TextInput.Text, config)
    end)

    -- Hover effects
    local inputButton = NeverZen:NewInput(InputFrame)
    
    inputButton.MouseEnter:Connect(function()
        if not TextInput:IsFocused() then
            TweenService:Create(UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
                Transparency = 0.1
            }):Play()
            
            TweenService:Create(InputFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
                BackgroundColor3 = Color3.new(
                    NeverZen.Theme.BackgroundColor.R + 0.02,
                    NeverZen.Theme.BackgroundColor.G + 0.02,
                    NeverZen.Theme.BackgroundColor.B + 0.02
                )
            }):Play()
        end
    end)
    ...
    return {
        SetValue = function(self, newvalue)
            TextInput.Text = tostring(newvalue or "")
            config.Callback(TextInput.Text, config)
        end,
        ...
        SetPlaceholder = function(self, newplaceholder)
            TextInput.PlaceholderText = tostring(newplaceholder or "")
        end,
...
function NeverZen:CreateNotifier()
	...
	return {
		new = function(head , body , duration)
			...
			local notifyGradient = NeverZen:CreateGradient(Notify, NeverZen.Theme.WindowGradient, 45)
			...
			Header.Text = head
			Header.TextColor3 = NeverZen.Theme.HeadText
